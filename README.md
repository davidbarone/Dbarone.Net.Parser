# Dbarone.Net.Parser
A .NET parser generator that can generate LL(1) parsers.
...
## Left Recursion
This library does **not** support left recursion. A rule is directly left recursive if it has a form where the left most symbol is itself, for example:
...
Additionally, any node can be pretty-printed to display the abstract syntax tree in a user-friendly format. The tree can be displayed for any node by calling:
